...
class AutorLibroController{
    ...
    static async putAutorLibro(req,res){
        try {
            ...
            res.status(500).json( {message: "Error al actualizar el AutorLibro" + error} );
        }
    }
    ...
    static async estadoAutorLibro(req, res) {
        try {
            ...
            res.status(200).json({ message: "Estado del AutorLibro alterado con éxito" });
          } catch (error) {
            res.status(500).json({ message: "Error al alterar el estado del AutorLibro: " + error.message });
          }
    }
